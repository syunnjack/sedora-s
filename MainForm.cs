...
public sealed class MainForm : Form
{
    ...
    private void ShowWork(Work w, string code, string type, MarketSummary market)
    {
        ...
        string surugaya = "https://www.suruga-ya.jp/search?search_word=" + Uri.EscapeDataString(code);
        string animate = "https://www.animate-onlineshop.jp/products/list.php?mode=search&smt=" + Uri.EscapeDataString(code);
        string marketLinks = $"<div class='market-links'><a class='button secondary' href='{E(surugaya)}' target='_blank'>駿河屋でJAN検索</a><a class='button secondary' href='{E(animate)}' target='_blank'>アニメイトで検索</a></div>";
        string body = $"<div class='meta'>{E(type)} ・ {E(code)} ・ {E(w.Category)} ・ 情報元: {E(w.Source)}</div><div class='result'>{image}<section><h1>{E(w.Title)}</h1>{Row("出演者・著者・販売元", w.Creator)}{Row("発売元・出版社", w.Publisher)}{Row("発売日", w.Date)}{(string.IsNullOrWhiteSpace(w.Description) ? "" : $"<p class='description'>{E(w.Description)}</p>")}{link}</section></div><h2>中古市場・販売相場の自動判定</h2><p><strong>{E(judgment)}</strong></p><table><tr><th>市場</th><th>状態</th><th>取得価格</th><th>件数</th></tr>{marketRows}</table><p>画面下の利益計算には推奨販売価格を自動入力しています。仕入額を入力すると利幅を判定できます。</p><h2>ホビー・アニメ専門市場</h2><p>駿河屋・アニメイトは公開商品APIが確認できないため、JANを引き継いだ公式検索を開きます。提携データ提供の許諾後、自動価格集計へ切り替えられる構成です。</p>{marketLinks}";
